...
#define ONET_DHCPCD_CONF  ONET_RUN_DIR "/dhcpcd.conf"
#define ONET_DHCPCD_PID   ONET_RUN_DIR "/dhcpcd.pid"
#define ONET_DHCPCD_LOG   ONET_LOG_DIR "/dhcpcd.log"
...
int ipv6_pd_start(const global_config_t *g, const char *lan_names_csv) {
    ...
    FILE *f = fopen(ONET_DHCPCD_CONF, "w");
    if (!f) {
        log_error("cannot write %s: %m", ONET_DHCPCD_CONF);
        return -1;
    }

    fprintf(f, "# Auto-generated by ONeT — do not edit\n");
    fprintf(f, "duid\n");
    ...
    /* Append each LAN with an incrementing sub-prefix index. */
    int idx = 0;
    if (lan_names_csv && lan_names_csv[0]) {
        char buf[512];
        snprintf(buf, sizeof buf, "%s", lan_names_csv);
        char *tok = strtok(buf, ",");
        while (tok) {
            while (*tok == ' ') tok++;
    ...
    /* Tell dhcpcd not to manage RA on the LAN ifaces — dnsmasq does that. */
    if (lan_names_csv && lan_names_csv[0]) {
        char buf[512];
        snprintf(buf, sizeof buf, "%s", lan_names_csv);
        char *tok = strtok(buf, ",");
        while (tok) {
            ...
            if (*tok) {
                fprintf(f, "interface %s\n", tok);
                fprintf(f, "  static ip6_address=auto\n");  /* dhcpcd assigns the /64 */
                fprintf(f, "  noipv6rs\n");                  /* don't send RA from dhcpcd */
            }
            tok = strtok(NULL, ",");
        }
    }
    ...
    if (idx == 0) {
        log_warn("PD enabled but no LAN ifaces declared IPv6=1; not starting dhcpcd");
        return 0;
    }
...
int ipv6_pd_stop(void) {
    return proc_kill_pidfile(ONET_DHCPCD_PID, SIGTERM);
}
